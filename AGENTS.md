...
**circle-skirt-calculator** is a browser app that works out the pattern
measurements for sewing a circle skirt. Enter your body measurements and the
skirt you want, and it gives you the numbers to cut.

> TODO: replace this with the real problem statement once
...
React 19 + TypeScript on Vite with Tailwind CSS v4, and npm as the package
manager.

- Dev server: `npm run dev` (http://localhost:5173)
- Build: `npm run build` (runs `tsc -b`, then `vite build` to `dist/`)
- Preview production build: `npm run preview`
- Lint: `npm run lint`
- Typecheck: `npx tsc -b` (the tsconfigs already set `noEmit`)
- Test: `npm test` (Vitest, single run)
- Test (watch): `npm run test:watch`
- Verify: none configured yet. Run `/ci` or `$ci` to define one.

The test command above is the logic-test gate, and it is now on. A step that adds
logic to `src/lib/` ships a passing test in the same diff.
